...
corr <- cor(db[,2:length(db)], use="complete.obs", method="pearson")
#corr
...
corr_di <- cor(db[2:17,2:length(db)], use="complete.obs", method="pearson")
pdf(paste("plots/",file_name[2],"_di_corr.pdf",sep = ""))
fig2 <- corrplot(corr_di, order = "hclust")
dev.off()

corr_tri <- cor(db[18:81,2:length(db)], use="complete.obs", method="pearson")
